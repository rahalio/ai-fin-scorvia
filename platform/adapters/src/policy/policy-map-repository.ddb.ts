/**
 * PolicyMapRepository - DynamoDB Implementation
 *
...
/**
 * PolicyMapRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing PolicyMap repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { PolicyMapRepository } from "@scorvia/services/policy";
...
/**
 * PolicyMapRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: POLICY_MAP
 * Entity Token: POLICY_MAP
 * SK Prefix: POLICY_MAP
 */
export class PolicyMapRepositoryDdb implements PolicyMapRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "POLICY_MAP";
  private readonly ENTITY_TOKEN = "POLICY_MAP";
  private readonly SK_PREFIX = "POLICY_MAP";
  ...
  private readonly DOMAIN = "POLICY";
  private readonly ENTITY_TYPE_PK = "POLICY_MAPS";
  private generateCorrelationId(): string {
    const domainPrefix = "pol";
  ...
  async getPolicyMap(input: Parameters<PolicyMapRepository['getPolicyMap']>[0]): Promise<Awaited<ReturnType<PolicyMapRepository['getPolicyMap']>>> {
    ...
    if (!(inputExt.id ?? inputExt.policyMapId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.policyMapId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.policyMapId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.policyMapId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
