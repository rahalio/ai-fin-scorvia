/**
 * ReferralRepository - DynamoDB Implementation
 *
...
/**
 * ReferralRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Referral repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { ReferralRepository } from "@scorvia/services/referrals";
...
/**
 * ReferralRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: REFERRAL
 * Entity Token: REFERRAL
 * SK Prefix: REFERRAL
 */
export class ReferralRepositoryDdb implements ReferralRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "REFERRAL";
  private readonly ENTITY_TOKEN = "REFERRAL";
  private readonly SK_PREFIX = "REFERRAL";
  ...
  private readonly DOMAIN = "REFERRALS";
  private readonly ENTITY_TYPE_PK = "REFERRALS";
  ...
  async getReferral(input: Parameters<ReferralRepository['getReferral']>[0]): Promise<Awaited<ReturnType<ReferralRepository['getReferral']>>> {
    ...
    if (!(inputExt.id ?? inputExt.referralId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.referralId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.referralId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.referralId)} not found`);
    }
    return {
