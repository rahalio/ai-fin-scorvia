/**
 * AutomlRunRepository - DynamoDB Implementation
 *
...
/**
 * AutomlRunRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing AutomlRun repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { AutomlRunRepository } from "@scorvia/services/automl";
...
/**
 * AutomlRunRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: AUTOML_RUN
 * Entity Token: AUTOML_RUN
 * SK Prefix: AUTOML_RUN
 */
export class AutomlRunRepositoryDdb implements AutomlRunRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "AUTOML_RUN";
  private readonly ENTITY_TOKEN = "AUTOML_RUN";
  private readonly SK_PREFIX = "AUTOML_RUN";
  ...
  private readonly ENTITY_TYPE_PK = "AUTOML_RUNS";
  ...
  async getAutoMLRun(input: Parameters<AutomlRunRepository['getAutoMLRun']>[0]): Promise<Awaited<ReturnType<AutomlRunRepository['getAutoMLRun']>>> {
    ...
    if (!(inputExt.id ?? inputExt.runId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.runId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.runId)}#`;
    const result = await this.dynamoClient.query({
      TableName: this.TABLE_NAME,
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.runId)} not found`);
    }
    return {
      data: sanitizeItem(item as Record<string, unknown>) as unknown as any,
      meta: {
