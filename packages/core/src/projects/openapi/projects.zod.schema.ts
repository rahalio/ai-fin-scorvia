import { makeApi, Zodios, type ZodiosOptions } from '@zodios/core';
import { z } from 'zod';

const createCreditProject_Body = z
  .object({
    name: z.string(),
    population: z.string(),
    environment: z.enum(['poc', 'production']),
    datasetId: z
      .string()
      .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
      .optional(),
    purpose: z.string().optional(),
    imbalanceRatio: z.number().optional(),
  })
  .passthrough();
const attachDataset_Body = z
  .object({
    name: z.string(),
    labelColumn: z.string().optional(),
    rowCount: z.number().int().optional(),
    imbalanceRatio: z.number().optional(),
  })
  .passthrough();
const Problem = z
  .object({
    type: z.string().url(),
    title: z.string(),
    status: z.number().int(),
    detail: z.string(),
    instance: z.string().url(),
    code: z.string(),
  })
  .partial()
  .passthrough();
const CreditProjectId = z.string();
const TenantId = z.string();
const ProjectStatus = z.enum(['draft', 'active', 'archived']);
const EnvironmentLabel = z.enum(['poc', 'production']);
const DatasetId = z.string();
const CreditProject = z
  .object({
    id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
    tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
    name: z.string(),
    status: z.enum(['draft', 'active', 'archived']),
    environment: z.enum(['poc', 'production']),
    population: z.string().optional(),
    imbalanceRatio: z.number().optional(),
    datasetId: z
      .string()
      .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
      .optional(),
    featureSetId: z.string().optional(),
    purpose: z.string().optional(),
    createdAt: z.string().datetime({ offset: true }),
    updatedAt: z.string().datetime({ offset: true }),
  })
  .passthrough();
const ResponseMeta = z
  .object({
    requestId: z.string().uuid(),
    correlationId: z.string(),
    generatedAt: z.string().datetime({ offset: true }),
  })
  .partial()
  .passthrough();
const CreditProjectListResponse = z
  .object({
    data: z
      .object({
        items: z.array(
          z
            .object({
              id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
              tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
              name: z.string(),
              status: z.enum(['draft', 'active', 'archived']),
              environment: z.enum(['poc', 'production']),
              population: z.string().optional(),
              imbalanceRatio: z.number().optional(),
              datasetId: z
                .string()
                .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
                .optional(),
              featureSetId: z.string().optional(),
              purpose: z.string().optional(),
              createdAt: z.string().datetime({ offset: true }),
              updatedAt: z.string().datetime({ offset: true }),
            })
            .passthrough()
        ),
        nextCursor: z.string().optional(),
      })
      .passthrough(),
    meta: z
      .object({
        requestId: z.string().uuid(),
        correlationId: z.string(),
        generatedAt: z.string().datetime({ offset: true }),
      })
      .partial()
      .passthrough()
      .optional(),
  })
  .passthrough();
const CreditProjectCreate = z
  .object({
    name: z.string(),
    population: z.string(),
    environment: z.enum(['poc', 'production']),
    datasetId: z
      .string()
      .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
      .optional(),
    purpose: z.string().optional(),
    imbalanceRatio: z.number().optional(),
  })
  .passthrough();
const CreditProjectResponse = z
  .object({
    data: z
      .object({
        id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
        tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
        name: z.string(),
        status: z.enum(['draft', 'active', 'archived']),
        environment: z.enum(['poc', 'production']),
        population: z.string().optional(),
        imbalanceRatio: z.number().optional(),
        datasetId: z
          .string()
          .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
          .optional(),
        featureSetId: z.string().optional(),
        purpose: z.string().optional(),
        createdAt: z.string().datetime({ offset: true }),
        updatedAt: z.string().datetime({ offset: true }),
      })
      .passthrough(),
    meta: z
      .object({
        requestId: z.string().uuid(),
        correlationId: z.string(),
        generatedAt: z.string().datetime({ offset: true }),
      })
      .partial()
      .passthrough()
      .optional(),
  })
  .passthrough();
const DatasetAttach = z
  .object({
    name: z.string(),
    labelColumn: z.string().optional(),
    rowCount: z.number().int().optional(),
    imbalanceRatio: z.number().optional(),
  })
  .passthrough();
const Dataset = z
  .object({
    id: z.string().regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/),
    tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
    projectId: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
    name: z.string(),
    labelColumn: z.string().optional(),
    rowCount: z.number().int().optional(),
    imbalanceRatio: z.number().optional(),
    createdAt: z.string().datetime({ offset: true }),
    updatedAt: z.string().datetime({ offset: true }),
  })
  .passthrough();
const DatasetResponse = z
  .object({
    data: z
      .object({
        id: z.string().regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/),
        tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
        projectId: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
        name: z.string(),
        labelColumn: z.string().optional(),
        rowCount: z.number().int().optional(),
        imbalanceRatio: z.number().optional(),
        createdAt: z.string().datetime({ offset: true }),
        updatedAt: z.string().datetime({ offset: true }),
      })
      .passthrough(),
    meta: z
      .object({
        requestId: z.string().uuid(),
        correlationId: z.string(),
        generatedAt: z.string().datetime({ offset: true }),
      })
      .partial()
      .passthrough()
      .optional(),
  })
  .passthrough();

export const schemas: any = {
  createCreditProject_Body,
  attachDataset_Body,
  Problem,
  CreditProjectId,
  TenantId,
  ProjectStatus,
  EnvironmentLabel,
  DatasetId,
  CreditProject,
  ResponseMeta,
  CreditProjectListResponse,
  CreditProjectCreate,
  CreditProjectResponse,
  DatasetAttach,
  Dataset,
  DatasetResponse,
};

const endpoints = makeApi([
  {
    method: 'get',
    path: '/v1/projects',
    alias: 'listCreditProjects',
    requestFormat: 'json',
    parameters: [
      {
        name: 'cursor',
        type: 'Query',
        schema: z.string().optional(),
      },
      {
        name: 'limit',
        type: 'Query',
        schema: z.number().int().gte(1).lte(200).optional().default(50),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            items: z.array(
              z
                .object({
                  id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
                  tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
                  name: z.string(),
                  status: z.enum(['draft', 'active', 'archived']),
                  environment: z.enum(['poc', 'production']),
                  population: z.string().optional(),
                  imbalanceRatio: z.number().optional(),
                  datasetId: z
                    .string()
                    .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
                    .optional(),
                  featureSetId: z.string().optional(),
                  purpose: z.string().optional(),
                  createdAt: z.string().datetime({ offset: true }),
                  updatedAt: z.string().datetime({ offset: true }),
                })
                .passthrough()
            ),
            nextCursor: z.string().optional(),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'post',
    path: '/v1/projects',
    alias: 'createCreditProject',
    requestFormat: 'json',
    parameters: [
      {
        name: 'body',
        type: 'Body',
        schema: createCreditProject_Body,
      },
      {
        name: 'Idempotency-Key',
        type: 'Header',
        schema: z.string().min(1).max(128),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
            tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
            name: z.string(),
            status: z.enum(['draft', 'active', 'archived']),
            environment: z.enum(['poc', 'production']),
            population: z.string().optional(),
            imbalanceRatio: z.number().optional(),
            datasetId: z
              .string()
              .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
              .optional(),
            featureSetId: z.string().optional(),
            purpose: z.string().optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 422,
        description: `Semantically invalid request (e.g. PACK_EMPTY)`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'get',
    path: '/v1/projects/:projectId',
    alias: 'getCreditProject',
    requestFormat: 'json',
    parameters: [
      {
        name: 'projectId',
        type: 'Path',
        schema: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            id: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
            tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
            name: z.string(),
            status: z.enum(['draft', 'active', 'archived']),
            environment: z.enum(['poc', 'production']),
            population: z.string().optional(),
            imbalanceRatio: z.number().optional(),
            datasetId: z
              .string()
              .regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/)
              .optional(),
            featureSetId: z.string().optional(),
            purpose: z.string().optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 404,
        description: `Resource not found`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'post',
    path: '/v1/projects/:projectId/datasets',
    alias: 'attachDataset',
    requestFormat: 'json',
    parameters: [
      {
        name: 'body',
        type: 'Body',
        schema: attachDataset_Body,
      },
      {
        name: 'projectId',
        type: 'Path',
        schema: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
      },
      {
        name: 'Idempotency-Key',
        type: 'Header',
        schema: z.string().min(1).max(128),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            id: z.string().regex(/^dst_[0-9A-HJKMNP-TV-Z]{26}$/),
            tenantId: z.string().regex(/^tnt_[0-9A-HJKMNP-TV-Z]{26}$/),
            projectId: z.string().regex(/^prj_[0-9A-HJKMNP-TV-Z]{26}$/),
            name: z.string(),
            labelColumn: z.string().optional(),
            rowCount: z.number().int().optional(),
            imbalanceRatio: z.number().optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 404,
        description: `Resource not found`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 422,
        description: `Semantically invalid request (e.g. PACK_EMPTY)`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
]);

export const api: any = new Zodios(
  'https://api.ddd-codegen-starter.local/v1',
  endpoints
);

export function createApiClient(baseUrl: string, options?: ZodiosOptions): any {
  return new Zodios(baseUrl, endpoints, options);
}
